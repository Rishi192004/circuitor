...
| `can_simulate` | `boolean` | ✅ | **True if the circuit is electrically valid for simulation.** (False if any `severity: "error"` issues exist). Use this to enable/disable your "Play" button. |
| `issues_count` | `number` | ✅ | Total number of strict validation issues found. |
| `issues` | `array` | ✅ | Array of validation issue objects (see below). |
| `suggestions_count` | `number` | ✅ | Total number of intent-based suggestions found. |
| `suggestions` | `array` | ✅ | Array of pattern-based suggestions (see Section 5 below). |
| `graph` | `object` | ✅ | Adjacency list of pin-to-pin connections. |
| `metadata` | `object` | ✅ | Includes `rules_run`, `patterns_run`, and `timestamp`. |
...
### Response with Intent Suggestions

Even if `status` is `"success"`, the backend may provide **suggestions** to improve the design.
  ...
  "suggestions": [
    {
      ...
      "type": "ADD_COMPONENT",
      "component": "resistor",
      "reason": "LED 'D1' is connected directly to source without a resistor.",
      "confidence": 0.95,
      "priority": 10,
      "target_component_ids": ["D1", "V1"],
      "metadata": { "net_id": "net_1", "suggested_value": "330" }
    }
  ],
  "..." : "..."
}
...
## 5. Pattern Suggestion Object (NEW)

Unlike `issues`, which represent errors, `suggestions` represent **intent-based improvements**. These are generated by the Pattern Engine and never block simulation.

### Suggestion Schema

| Field | Type | Description |
|---|---|---|
| `pattern_id` | `string` | ID of the detected pattern (e.g., `"LED_MISSING_RESISTOR"`). |
| `type` | `string` | Action type: `"ADD_COMPONENT"`, `"ADD_CONNECTION"`, or `"INSPECT_NODE"`. |
| `component` | `string` | (Optional) Recommendation type (e.g., `"resistor"`). |
| `reason` | `string` | User-friendly explanation of why this suggestion was made. |
| `confidence` | `number` | Confidence score (0.0 to 1.0). Use to hide low-confidence hints. |
| `priority` | `number` | Lower is more urgent (e.g., 10=Safety, 30=Optimization). |
| `target_component_ids` | `array` | List of component IDs related to the pattern. |
| `metadata` | `object` | Key-value pairs (e.g., `{"suggested_value": "330"}`). |

### Consuming Suggestions in UI
1. **Ghost Nodes:** Use `type: "ADD_COMPONENT"` to render a semi-transparent "ghost" resistor on the canvas.
2. **Badge counts:** Show a subtle badge (e.g., "3 Hints") instead of red error icons.
3. **Tab-to-Accept:** If your UI supports it, let users Tab through suggestions and Enter to auto-place.
